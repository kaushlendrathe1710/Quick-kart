...
/**
 * Delivery Partner Ticket Controller
 * Handles support ticket operations for delivery partners
 */
export class DeliveryPartnerTicketController {
  /**
   * Get ticket by ID (own tickets only)
   * GET /api/delivery-partner/tickets/:id
   */
  static async getTicketById(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      // Check if delivery partner owns this ticket
      if (ticket.deliveryPartnerId !== userId) {
  ...
  /**
   * Get all tickets for current delivery partner
   * GET /api/delivery-partner/tickets
   */
   ...
   * POST /api/delivery-partner/tickets
   */
